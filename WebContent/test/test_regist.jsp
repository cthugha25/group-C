		...
		<section class="mo-4">
				...
				<div class="row border mx-3 mb-3 py-2 align-items-center rounded" id="filter">
					...
					<div class="col-4">
						...
						<select class="form-select" id ="student-f2-select" name="f4">
							...
							<c:forEach var="num" items="${class_num_set }">
								<%--現在のnumと選択されていたf3が一致していた場合selectedを追記 --%>
								<option value="${num.classNum }" <c:if test="${num.classNum==f4 }">selected</c:if>>${num.classNum }</option>
							</c:forEach>
						</select>
					</div>
			...
			<c:choose>
				<c:when test="${tests.size()>0}">
					<form method="get" action="Student_filter">
						<div>科目：${test.subject}（${test.no}回）</div>
						<table class="table table-hover">
							...
							<c:forEach var="student" items="${students}">
								<tr>
									...
									<td>${student.name}</td>
									<td>
										<input type="number" name="point_${student.no}" min="1" max="100">
									</td>
								</tr>
							</c:forEach>
						</table>
